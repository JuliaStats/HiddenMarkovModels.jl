...
## Internal sampling helpers — kept module-private so they can evolve without API churn.
...
function StatsAPI.fit!(
    ...
    if var_x <= mean_x || mean_x <= 0
        # Underdispersed (or degenerate) — MLE pushes r → ∞. Pick something stable.
        d.p = oftype(d.p, 0.5)
    ...
    for _ in 1:100
        ...
        # The W·log W term is constant in r (so absent from g'), but it's part of the
        # score — dropping it shifts the zero of g and breaks Newton convergence.
